 ...
 //                 Serial.println("End of HTTP Request");
  ...
  if (br_level > 970 && pir_state) {
    ...
    if (count_br_level > 500 && !pr_light_Hall){
      ...
      // if all  lights is on -> pr_light_Hall = true, time_light_Hall is start ---
      if (pr_light_RPi && pr_light_Hue1 && pr_light_Hue2){
        time_light_Hall = millis();
        pr_light_Hall = true;
      ...
      else if (pr_light_RPi || pr_light_Hue1 || pr_light_Hue2){
        time_light_Hall = millis();
        pr_light_Hall = true;
        Serial.println("Some lights in Hall is On");
      }
    }
  } else {
    count_br_level = 0;
